  ...
  return (
    <header ref={menuRef} className="relative bg-white/80 backdrop-blur-md border-b border-white/20 sticky top-0 z-50">
      <div className="px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
        <Link
          to="/"
          className={`text-xl font-bold bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 bg-clip-text text-transparent hover:scale-105 transition-all duration-500 flex-shrink-0 ${isLoaded ? "animate-fade-in-left" : "opacity-0"}`}
        >
          GoodPricePC
        </Link>
        ...
        <div className={`flex items-center gap-2 flex-shrink-0 ${isLoaded ? "animate-fade-in-right" : "opacity-0"}`}>
          ...
          <Link to="/favorites" className="p-2 rounded-lg text-slate-600 hover:bg-slate-100 transition-colors" aria-label="즐겨찾기">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          >
            <div className="w-6 h-6 flex flex-col justify-center space-y-1">
              <div className={`h-0.5 bg-slate-600 transition-all duration-300 ${isMenuOpen ? "rotate-45 translate-y-1.5" : ""}`} />
              <div className={`h-0.5 bg-slate-600 transition-all duration-300 ${isMenuOpen ? "opacity-0" : ""}`} />
              <div className={`h-0.5 bg-slate-600 transition-all duration-300 ${isMenuOpen ? "-rotate-45 -translate-y-1.5" : ""}`} />
            </div>
          </button>
        </div>
      ...
      <div
        ...
        } border-t border-white/20 bg-white/90 backdrop-blur-md`}
      >
        <form onSubmit={handleSearchSubmit} className="px-4 sm:px-6 lg:px-8 py-3 flex gap-2">
          <input
            ...
            className="flex-1 px-4 py-2 text-sm rounded-lg border border-slate-200 bg-white text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-400"
          />
      ...
      <div
        className={`md:hidden absolute top-full left-0 right-0 bg-white/95 backdrop-blur-md border-b border-white/20 shadow-lg transition-all duration-300 ${
      ...
      >
        <nav className="px-4 py-5">
          ...
          <div className="border-t border-slate-200 pt-3 flex flex-wrap gap-2">
            <NavLink to="/guide" className="px-4 py-2 text-sm font-medium rounded-lg bg-slate-100 text-slate-700 hover:bg-slate-200 transition-colors" onClick={closeMenu}>
              📖 가이드
            </NavLink>
