// routes/updatePrices.js - 개선된 다나와 크롤링 버전
...
async function crawlDanawaCategory(category) {
  ...
  try {
    ...
    const allProducts = [];
    const maxPages = 5; // ⭐ 5페이지까지 크롤링 (기존: 1페이지)
...
function findLowestPriceByProductName(dbPart, crawledProducts) {
  ...
  // ⭐ 개선: 70% 유사도로 하향 (기존: 80%)
  const matchingProducts = similarities.filter((s) => s.similarity >= 0.7);

  if (matchingProducts.length === 0) {
    console.log(`   ⛔ 유사 제품 없음 (유사도 < 70%)`);
    return null;
  }
...
router.post("/update-prices", async (req, res) => {
  try {
    res.json({
      message: "✅ 다나와 가격 업데이트 시작 (개선 버전)",
      info: "백그라운드에서 크롤링 진행 중입니다. 완료까지 10-15분 소요됩니다."
    });
