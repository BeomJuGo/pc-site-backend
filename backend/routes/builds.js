...
// shareId 중복 방지: 최대 3회 재시도, 6바이트(12자리 hex) → 약 281조 조합
async function generateUniqueShareId(db) {
  for (let i = 0; i < 3; i++) {
    const shareId = crypto.randomBytes(6).toString("hex");
    const existing = await db.collection("builds").findOne({ shareId }, { projection: { _id: 1 } });
    if (!existing) return shareId;
  }
  throw new Error("shareId 생성 실패: 재시도 횟수 초과");
}
...
router.get("/:shareId/pdf", async (req, res) => {
  try {
    ...
    for (const item of items) {
      ...
      const safeName = name.replace(/[^\x20-\x7E]/g, () => "?").substring(0, 60);
