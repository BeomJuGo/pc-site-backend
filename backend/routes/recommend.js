...
export async function buildCompatibleSetWithAIV2(budget, db) {
  ...
  const isLow  = budget < 700000;
  const isHigh = budget >= 1300000;
  ...
  // 가격 범위 내 인기순 (범위 내 없으면 상한만 적용)
  const popularInRange = (arr, category, priceMin, priceCap) => {
    const inRange = popularFirst(arr, category, priceCap).filter(p => p.price >= priceMin);
    return inRange.length > 0 ? inRange : popularFirst(arr, category, priceCap);
  };

  // ─── Phase 1: 보조 부품 (예산 비례 가격 범위) ────────────────────────────
  // 예산 구간별 보조부품 비중: 저예산 25%, 중 20%, 고 17%
  const secBudget = budget * (isLow ? 0.25 : isHigh ? 0.17 : 0.20);

  const preStorage = popularInRange(storages, "storage", secBudget * 0.10, Math.min(secBudget * 0.38, budget * 0.13))[0];
  const prePsu     = popularInRange(psus,     "psu",     secBudget * 0.08, Math.min(secBudget * 0.34, budget * 0.12))[0];
  const preCase    = popularInRange(cases,    "case",    secBudget * 0.06, Math.min(secBudget * 0.26, budget * 0.09))[0];
  const preCooler  = popularInRange(coolers,  "cooler",  secBudget * 0.04, Math.min(secBudget * 0.20, budget * 0.07))[0];
  ...
  // ─── Phase 2: CPU 후보 다양화 (가성비 + 고사양) ──────────────────────────
  const boardCap = Math.max(remainingBudget * 0.27, 80000);
  const memCap   = Math.max(remainingBudget * 0.20, 55000);

  // 가성비 CPU: score/price 상위 (저예산 최적)
  const valueCpus = [...cpus]
    .filter(p => p.price > 0 && getCpuScore(p) > 0 && p.price <= remainingBudget * 0.35)
    .sort((a, b) => (getCpuScore(b) / b.price) - (getCpuScore(a) / a.price))
    .slice(0, 5);

  // 고사양 CPU: 절대 성능순, 예산 15~45% 구간 (고예산 최적)
  const premiumCpus = [...cpus]
    .filter(p => p.price > 0 && p.price >= remainingBudget * 0.15 && p.price <= remainingBudget * 0.45)
    .sort((a, b) => (getCpuScore(b) - getCpuScore(a)) || (b.price - a.price))
    .slice(0, 5);

  const candidateCpusArr = [
    ...new Map([...valueCpus, ...premiumCpus].map(c => [c.name, c])).values(),
  ];
  if (candidateCpusArr.length < 3) {
    [...cpus]
      .filter(p => p.price > 0 && p.price <= remainingBudget * 0.5)
      .sort((a, b) => a.price - b.price)
      .slice(0, 5)
      .forEach(c => { if (!candidateCpusArr.find(x => x.name === c.name)) candidateCpusArr.push(c); });
  }

  const cpuCombos = [];
  for (const cpu of candidateCpusArr) {
  ...
  // 가격 오름차순 인덱스도 준비 (정확한 범위 탐색용)
  const gpuByPrice = [...gpuCandidates].sort((a, b) => a.price - b.price);

  // ─── Phase 4: AI에게 CPU 조합 + GPU 선택 요청 ─────────────────────────────
  const systemPrompt = `PC 견적 전문가입니다. 아래 조합 중 예산 범위에 정확히 드는 것을 선택하세요.
...
- 각 조합의 [GPU필요범위] 안에 드는 GPU를 선택해야 예산 범위 충족 가능
- 범위 내 GPU가 없으면 gpuName을 null로 설정 (단 총합이 범위 안이어야 함)`;

  const comboLines = cpuCombos.map((c, i) => {
    const gpuNeedMin = minBudget - secondaryTotal - c.comboPrice;
    const gpuNeedMax = maxBudget - secondaryTotal - c.comboPrice;
    return `조합${i}: ${c.cpu.name}(${c.cpu.price.toLocaleString()})+${c.board.name}(${c.board.price.toLocaleString()})+${c.mem.name}(${c.mem.price.toLocaleString()})=합계${c.comboPrice.toLocaleString()}원 [GPU필요범위:${gpuNeedMin.toLocaleString()}~${gpuNeedMax.toLocaleString()}원]`;
  }).join("\n");

  const gpuLines = gpuByPrice.slice(0, 14).map(g => `${g.name}(${g.price.toLocaleString()}원)`).join(" / ");

  const userPrompt = [
    `총예산:${budget.toLocaleString()}원 | 고정부품:${secondaryTotal.toLocaleString()}원`,
    `(저장=${preStorage.name} ${preStorage.price.toLocaleString()}원, 파워=${prePsu.name} ${prePsu.price.toLocaleString()}원, 케이스=${preCase.name} ${preCase.price.toLocaleString()}원, 쿨러=${preCooler.name} ${preCooler.price.toLocaleString()}원)`,
    "", "[CPU+메인보드+메모리 조합 — GPU필요범위 확인 후 선택]", comboLines,
    "", "[GPU 후보(가격오름차순)]", gpuLines,
  ].join("\n");
  ...
  // ─── Phase 5: 예산 범위 내 최적 조합 전수 탐색 ────────────────────────────
  // AI 제안 검증: 총합이 [minBudget, maxBudget) 이면 채택
  let chosen = null, chosenGpu = null;

  if (parsed && typeof parsed.comboIndex === "number") {
    const aiIdx = Math.min(Math.max(parsed.comboIndex, 0), cpuCombos.length - 1);
    const aiCombo = cpuCombos[aiIdx];
    const aiGpu = (parsed.gpuName && parsed.gpuName !== "null")
      ? gpuCandidates.find(g => g.name === parsed.gpuName)
      : null;
    const aiTotal = secondaryTotal + aiCombo.comboPrice + (aiGpu?.price || 0);
    if (aiTotal >= minBudget && aiTotal < maxBudget) {
      chosen = aiCombo;
      chosenGpu = aiGpu;
    }
  }

  // AI 결과가 범위 밖이거나 없으면 전수 탐색
  if (!chosen) {
    let bestDiff = Infinity;

    for (const combo of cpuCombos) {
      const baseTotal = secondaryTotal + combo.comboPrice;
      if (baseTotal >= maxBudget) continue;   // 조합만으로 이미 초과

      const gpuNeedMin = minBudget - baseTotal;  // GPU 가격 하한
      const gpuNeedMax = maxBudget - baseTotal;  // GPU 가격 상한 (exclusive)

      // ① 정확히 예산 범위 내 GPU 탐색 (있으면 즉시 채택)
      const exactGpu = gpuByPrice
        .filter(g => g.price >= Math.max(gpuNeedMin, 50000) && g.price < gpuNeedMax)
        .sort((a, b) => b.price - a.price)[0];

      if (exactGpu) {
        const total = baseTotal + exactGpu.price;
        const diff = Math.abs(total - (minBudget + maxBudget) / 2);
        if (diff < bestDiff) { bestDiff = diff; chosen = combo; chosenGpu = exactGpu; }
        if (bestDiff === 0) break;
        continue;
      }

      // ② 범위 내 GPU 없음 — 초과하지 않는 가장 비싼 GPU
      const bestUnder = gpuByPrice
        .filter(g => g.price > 0 && g.price < gpuNeedMax)
        .sort((a, b) => b.price - a.price)[0];

      const total = baseTotal + (bestUnder?.price || 0);
      // diff: 미달이면 부족액, 초과면 초과액+1 (미달을 선호)
      const diff = total < minBudget ? (minBudget - total) * 1 : (total - maxBudget + 1) * 2;

      if (diff < bestDiff) { bestDiff = diff; chosen = combo; chosenGpu = bestUnder || null; }
    }
  ...
  logger.info(`[V2] 예산 ${budget.toLocaleString()}원 → 총합 ${finalTotal.toLocaleString()}원 (${finalTotal >= minBudget && finalTotal < maxBudget ? "✅범위내" : "⚠️범위외"})`);
