...
async function saveToMongoDB(coolers, { ai = true, force = false } = {}) {
  ...
  for (const cooler of coolers) {
    ...
    if (ai) {
      ...
      if (needsAI) {
        try {
          const aiRes = await callGptInfo(cooler.name, "cooler", "gpt-5.4-mini", OPENAI_API_KEY);
          if (aiRes.review) review = aiRes.review;
          if (aiRes.specSummary) specSummary = aiRes.specSummary;
        } catch (e) {
          console.error(`AI \uC0DD\uC131 \uC2E4\uD328: ${cooler.name} \u2014 ${e.message}`);
        }
      }
    }
    ...
    if (old) {
      ...
      await col.insertOne({ name: cooler.name, ...update, price: cooler.price, mallCount: 0, priceHistory: cooler.price > 0 ? [{ date: today, price: cooler.price }] : [] });
      inserted++;
