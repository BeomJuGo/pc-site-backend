...
app.get("/api/naver-price", async (req, res) => {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`ㅇ ${response.status}`);

    const data = await response.json();
    res.json(data);
  } catch (error) {
    console.error("❌ 네이버 API 오류:", error);
    res.status(500).json({ error: "네이버 가격 검색 실패" });
  }
});
...
const fetchCpuBenchmark = async (cpuName) => {
  try {
    ...
    if (scores.length === 0) throw new Error("점수를 찾을 수 없습니다.");
